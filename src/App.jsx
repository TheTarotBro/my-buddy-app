    ...
    <div style={{ minHeight: "100vh", background: "#f5f0ea", fontFamily: "'Inter', -apple-system, sans-serif", color: "#3a2e24", maxWidth: 480, margin: "0 auto" }}>
      ...
      {settings && (
        <div style={{ margin: "4px 20px 8px", padding: 14, background: "rgba(90,74,62,0.03)", borderRadius: 12, border: "1px solid rgba(90,74,62,0.08)", animation: "fi 0.2s ease" }}>
          ...
          {[{ k: "water", l: "Water (oz)", i: "⬤" }, { k: "sleep", l: "Sleep (hrs)", i: "⬤" }, { k: "meals", l: "Meals", i: "⬤" }].map((g, gi) => (
            <div key={g.k} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              <span style={{ fontSize: 8, color: ["#5baed6","#9c7cb8","#6ee7a0"][gi], width: 10 }}>{g.i}</span>
              <span style={{ fontSize: 12, fontWeight: 500, color: "#8a7a6a", width: 90 }}>{g.l}</span>
      ...
      <div style={{ display: "flex", justifyContent: "center", padding: "0", position: "relative", margin: "0 20px 6px", borderRadius: 16, overflow: "hidden", height: 220, background: "#f5f0ea" }}>
        ...
        <div style={{ position: "absolute", bottom: 8, zIndex: 1, fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: mood === "happy" ? accent : mood === "content" ? "rgba(255,255,255,0.8)" : "#e86a6a", background: "rgba(0,0,0,0.4)", padding: "3px 12px", borderRadius: 8, backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)" }}>{mood === "happy" ? "Happy" : mood === "content" ? "Content" : "Needs care"}</div>
      </div>
